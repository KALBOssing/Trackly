...
export async function DELETE(_req: Request, { params }: { params: { resourceId: string } }) {
  ...
  const resource = await prisma.pathwayResource.findUnique({
    where: { id: params.resourceId },
    include: { lessonPathway: { include: { lesson: true } } },
  });
  if (!resource || resource.lessonPathway.lesson.teacherId !== session.user.teacherProfileId) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  await prisma.pathwayResource.delete({ where: { id: resource.id } });
